...
def make_mock_redis(get_return=None):
    """Return a mock that acts as an async context manager for aioredis."""
    ...
    cm = MagicMock()
    cm.__aenter__ = AsyncMock(return_value=mock_redis)
...
class TestSessionManager:
    async def test_get_returns_none_when_missing(self):
        cm, _ = make_mock_redis(get_return=None)
    ...
    async def test_get_deserialises_session(self, sample_session):
        ...
        cm, _ = make_mock_redis(get_return=json_data)
        with patch("app.services.session_manager._get_redis", return_value=cm):
